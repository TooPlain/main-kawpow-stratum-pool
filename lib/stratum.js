...
var StratumClient = function(options){
    ...
    function handleAuthorize(message){
         console.log("Handing authorize" + JSON.stringify(message));

        _this.workerName = getSafeWorkerString(message.params[0]);
        _this.workerPass = message.params[1];
		//logging debugger
		console.log(_this.remoteAddress + ' ' +  options.socket.localPort+' '+ _this.workerName+' '+ _this.workerPass+' '+ _this.extraNonce1+' '+ _this.version);
